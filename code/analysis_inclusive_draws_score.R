## FALLBACK VERSION (review/step2_analysis_plan.md, section 1.2, last bullet): the covariates enter
## the membership model as one prognostic score (linear predictor of a Cox model without phenotype)
## because the full-covariate membership model did not finish in reasonable time.
...
for (m in c("C3", "C4")) {
  covs <- get(m)
  cx <- coxph(as.formula(paste("Surv(time, event) ~", covs)), data = L)       # prognostic score, no phenotype
  Ls$score <- as.numeric(scale(predict(cx, newdata = L, type = "lp")))
  f <- as.formula(paste0("cbind(", paste(ITEMS, collapse = ","), ") ~ event + H_NA + score"))
  set.seed(20260918)
  t0 <- Sys.time()
  ## 10 random starts, as in the original analysis: with covariates a single start can converge
  ## to a degenerate solution with an empty class
  fi <- poLCA(f, data = Ls, nclass = 4, maxiter = 8000, nrep = 10, verbose = FALSE, calc.se = FALSE)
...
write.csv(do.call(rbind, rows), file.path(OUT, "RR5_inclusive_draws_score.csv"), row.names = FALSE)
write.csv(do.call(rbind, prof), file.path(OUT, "RR5_inclusive_profiles_somatic_depressive_score.csv"), row.names = FALSE)
