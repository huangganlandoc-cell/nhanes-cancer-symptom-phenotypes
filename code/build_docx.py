...
# ---------------------------------------------------------------- inserters
def insert_figure(doc, key, legends):
    ...
    caption(doc, key, legends.get(key, ''))


def insert_table(doc, key, legends):
...
def build_additional_file(md_path='manuscript/Manuscript.md', out='Additional_file_1.docx'):
    """BMC Cancer expects the supplementary material as one additional file: supplementary figures
    S1 to S3 followed by supplementary tables S1 to S13, each with the legend from the manuscript."""
    import glob
    md = open(md_path, encoding='utf-8').read()
    legends = parse_legends(md)
    doc = Document()
    ...
    h = doc.add_heading('Additional file 1', level=1)
    for r in h.runs:
        r.font.size = Pt(14); r.font.color.rgb = RGBColor(0, 0, 0)
    para(doc, 'Supplementary figures S1 to S3 and supplementary tables S1 to S13 for '
